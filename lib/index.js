...
function sheet_from_array_of_arrays(data, headColorBackground, headColorFont, columnsWidths) {
  ...
  // result.payload.data.entrega.filter(x => x.estaPlan === 'REPROGRAMADO').map(row => row.codPlanEntrVehi)

  var arregglo = [];
  for (var i = 0; i != columnsWidths.length; ++i) {
    console.log(columnsWidths[i]);
    arregglo.push({ "wch": columnsWidths[i] });
  }

  console.log(arregglo);
  var ws = {};
  ...
  for (var R = 0; R != data.length; ++R) {
    for (var C = 0; C != data[R].length; ++C) {
      ...
      console.log('headColorBackground:' + headColorBackground);
      console.log('headColorFont:' + headColorFont);
      if (R == 0) {
        cell.s = {
          fill: {
            // fgColor:{ rgb: "7d7d7d" }
